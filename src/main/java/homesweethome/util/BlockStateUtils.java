...
public class BlockStateUtils
{
    ...
    {
        if (!(block instanceof IHSHBlock)) {return ImmutableSet.<IBlockState>of();}
        IBlockState defaultState = block.getDefaultState();
        if (defaultState == null) {defaultState = block.getBlockState().getBaseState();}
        return getStatesSet(defaultState, ((IHSHBlock)block).getPresetProperties());        
    }    
    ...
    public static IBlockState getPresetState(IBlockState state)
    {
        ...
        if (state.getBlock() instanceof IHSHBlock)
        {
            IHSHBlock bopBlock = (IHSHBlock)state.getBlock();
